...
| The CORS policy allows the identity headers, not only the origin | `REQ-F-010`, `REQ-F-101`–`REQ-F-106` | `X-User-Id` makes every call a preflighted request, and a policy naming only the origin fails the preflight. The failure is invisible to `curl`, so every non-browser test exonerates the server |
